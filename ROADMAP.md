...
- [x] **`scripts/generate_pairs.py`** — run the 9 policies on the pair seed block and write the records stage 1b consumes. *(FEATURE_011, D-045/D-046, 2026-09-05 — 15 tests. Two modes. `--survey` runs all 21 variants — the 6 single-artefact policies plus 5 repeats each of dqn/reinforce/actor_critic — on seeds 3000000..3000011 and reports which are behaviourally indistinguishable; `--write` builds the pair set from the chosen repeats. **The repeat decision was taken from the survey, and the survey overturned the eval-seed picture that framed it:** on the pair seeds `reinforce@2` and `reinforce@4` take identical actions to `severity_sort` on all 12 seeds, but `reinforce@1` does not — it has the highest recall in the whole pool (0.8947 vs severity_sort's 0.8530), and the eval-seed table had it tied with the two clones. Chosen: repeat 0 for all three. Produced 108 records and 300 pairs (50 double-labelled), read end to end through Diya's `labelling/queue.py` — 175 pairs each for L1 and L2.)*
- [x] Labelling UI *(**Diya's box**, FEATURE_012, 2026-09-05 — 79 tests across `queue.py`, `render.py`, `app.py`, plus 11 config tests. Reads `results/rlhf/pairs.json` through `labelling/queue.py`, refuses `pairs_key.json` by name (D-038), writes through `rlhf/store.py`. Labeller id bound at launch, never in the request (D-041); timer capped and re-checked server-side (D-042, and D-044 for the NaN gap the cap missed). Tested against fixtures and verified end to end with a real `uvicorn` serve — `scripts/generate_pairs.py` below is still what produces the real `pairs.json`.)*
