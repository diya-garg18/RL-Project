...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Render a learned policy as a table.")
    parser.add_argument("--agent", default="q_learning",
                        choices=["q_learning", "sarsa", "monte_carlo"])
    args = parser.parse_args()

    q_path = ROOT / "results" / f"{args.agent}_Q.npy"
    v_path = ROOT / "results" / f"{args.agent}_visits.npy"
    if not q_path.exists() or not v_path.exists():
        raise SystemExit(
            f"results/{args.agent}_Q.npy or {args.agent}_visits.npy missing — run "
            f"`python scripts/train.py --agent {args.agent}` first (results/ is gitignored)."
        )
    ...
    out: list[str] = []
    out.append(f"# Learned policy — {args.agent} (run 0)\n")
    ...
    path = ROOT / "results" / f"policy_table_{args.agent}.md"
